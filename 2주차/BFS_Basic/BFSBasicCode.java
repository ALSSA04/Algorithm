...
public class BFSBasicCode {
    ...
    private static void bfs(int startR, int startC) {
        Queue<Point> q = new LinkedList(); //방문해야 할 좌표를 담을 Queue 생성
        q.add(new Point(startR, startC)); //Queue 에 시작점을 집어넣는다.
        visited[startR][startC] = true; //시작점에 대한 방문 표기

        while(!q.isEmpty()) { //방문해야 할 좌표가 존재한다면
            Point cur = q.poll(); //방문해야 할 좌표를 꺼낸다.
